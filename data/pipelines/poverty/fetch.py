...
Attempts to download DWP Households Below Average Income (HBAI) data.
Falls back gracefully; transform.py uses hardcoded data if download fails.
"""
...
def main():
    ...
    # DWP HBAI summary tables — URL changes with each annual release
    # Falls back to hardcoded data in transform.py
    any_new |= try_download(
        "hbai_summary",
        "https://stat-xplore.dwp.gov.uk/webapi/dataset/hbai/data",
        ".csv",
    )
